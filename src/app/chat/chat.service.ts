...
export class ChatService {
  ...
  async conectar(recibeMensaje) {
    ...
    .where('recibe', '==', user1).where('estado', '==', recibe)).valueChanges({idField: 'id' }).pipe(map(m => {
      m.map((data: any) => {
       this.afs.doc(`mensajes/${data.id}`).update({estado: 'leido'});
      });
    }), take(1)).subscribe();

    this.afs.collection(`chats`, ref => ref.where('uid', '==', this.room)).valueChanges().pipe(map(m => {
      if (m.length > 0){
